...
function OptionsUpdate($data, $siteId)
{
	...
	\Encoding\PhpArray\Write($fname, [
		...
		'redirect_towww' => $data['redirect_towww'],
...
function HandlerRedirectUrl()
{
	...
	if ('Y' == $currentOptions['redirect_www']
			&& 'www.' == substr($_SERVER['HTTP_HOST'], 0, 4))
	{
		$host = substr($_SERVER['HTTP_HOST'], 4);
		$url = $currentUri;
	} else {

		if ('Y' == $currentOptions['redirect_towww'] && (substr($_SERVER['HTTP_HOST'], 0, 4) !== 'www.') ) {
...
function init()
{
	...
	AddEventHandler('main', 'OnAdminListDisplay', function (&$list) {
		...
		{
			$url = $urlSettings . '&hdden_action=settings&site_id='
				. $row->arRes['LID'];
			$row->aActions[] = [
				'TEXT' => Loc::getMessage('HDDEN_REDIRECT_MODULE_NAME')
					. Loc::getMessage('HDDEN_REDIRECT_MENU_ITEM_SETTINGS'),
				'ACTION' => 'BX.SidePanel? BX.SidePanel.Instance.open("' . $url . '") : (location.href="' . $url . '");',
			];
			$url = $urlSettings . '&hdden_action=redirects&site_id='
				. $row->arRes['LID'];
			$row->aActions[] = [
				'TEXT' => Loc::getMessage('HDDEN_REDIRECT_MODULE_NAME')
					. Loc::getMessage('HDDEN_REDIRECT_MENU_ITEM_REDIRECTS'),
				'ACTION' => 'BX.SidePanel? BX.SidePanel.Instance.open("' . $url . '") : (location.href="' . $url . '");',
			];
		}
	});
